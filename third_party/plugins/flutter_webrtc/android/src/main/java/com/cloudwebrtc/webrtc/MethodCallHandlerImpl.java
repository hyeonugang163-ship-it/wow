...
public class MethodCallHandlerImpl implements MethodCallHandler, StateProvider {
  ...
  public void peerConnectionAddStream(final String streamId, final String id, Result result) {
    ...
    PeerConnectionObserver observer = mPeerConnectionObservers.get(id);
    if (observer == null || observer.getPeerConnection() == null) {
      resultError("peerConnectionAddStream", "peerConnection is null", result);
      return;
    }

    final List<String> streamIds = Collections.singletonList(streamId);

    Result silentResult = new Result() {
      @Override
      public void success(Object o) {
        // Intentionally no-op: addStream does not report sender info.
      }

      @Override
      public void error(String errorCode, String errorMessage, Object errorDetails) {
        Log.w(TAG, "peerConnectionAddStream(): addTrack error - " + errorCode + " " + errorMessage);
      }
    ...
    boolean anyTrackAdded = false;

    for (AudioTrack audioTrack : mediaStream.audioTracks) {
      observer.addTrack(audioTrack, streamIds, silentResult);
      anyTrackAdded = true;
    }

    for (VideoTrack videoTrack : mediaStream.videoTracks) {
      observer.addTrack(videoTrack, streamIds, silentResult);
      anyTrackAdded = true;
    }

    Log.d(TAG, "peerConnectionAddStream(): added tracks for stream " + streamId);
    result.success(anyTrackAdded);
  }
  ...
  public void peerConnectionRemoveStream(final String streamId, final String id, Result result) {
    ...
    for (RtpSender sender : peerConnection.getSenders()) {
      ...
      if (track != null &&
              (mediaStream.audioTracks.contains(track) || mediaStream.videoTracks.contains(track))) {
        boolean removed = peerConnection.removeTrack(sender);
        removedAny = removed || removedAny;
      }
    }

    Log.d(TAG, "peerConnectionRemoveStream(): removed tracks for stream " + streamId);
    result.success(removedAny);
  }
